...
public:
	...
	HRESULT STDMETHODCALLTYPE BeginEnableContent(IMFActivate *pEnablerActivate,
												 IMFTopology *pTopo,
		...
		if (MFENABLETYPE_WMDRMV1_LicenseAcquisition == gidEnableType ||
			MFENABLETYPE_WMDRMV7_LicenseAcquisition == gidEnableType) {
		...
		} else if (MFENABLETYPE_MF_UpdateUntrustedComponent == gidEnableType ||
			MFENABLETYPE_MF_UpdateRevocationInformation == gidEnableType) {
	...
	virtual STDMETHODIMP QueryInterface(REFIID riid, _COM_Outptr_ void **ppvObject) {
		if (!ppvObject) {
			return E_POINTER;
		}
	...
	void OnEnableEvent(IMFAsyncResult *pResult) {
		...
		if (MEEnablerCompleted == met) {
			if (SUCCEEDED(hrEvent)) {
				...
				m_pApp->HandleMMError(LoadAtlString(IDS_LICENSE_ACQUIRED), hr);
			}
