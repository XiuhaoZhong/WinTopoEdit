...
void CTedTransformDialog::PopulateCategory(DMOCategory &category) {
	...
	IMFActivate** ppAvtivates = NULL;
	UINT32 cMFTs = 0;
	::MFTEnumEx(category.m_GUID, MFT_ENUM_FLAG_ALL, NULL, NULL, &ppAvtivates, &cMFTs);

	for (WORD i = 0; i < cMFTs; i++) {
		m_Activates.Add(ppAvtivates[i]);

		LPWSTR pszName = NULL;
		ppAvtivates[i]->GetAllocatedString(MFT_FRIENDLY_NAME_Attribute, &pszName, NULL);

		m_strNames.Add(CAtlString(pszName));
