...
class CTedAppVideoWindowHandler : public ITedVideoWindowHandler {
public:
...
class CTedChooserDialog : public CDialogImpl<CTedChooserDialog> {
public:
	...
	void AddProcessibleChoice(CAtlStringW strChoice);
...
class CTedAboutDialog : public CDialogImpl<CTedAboutDialog> {
public:
	enum {
		IDD = IDD_ABOUTBOX
	};
...
class CTedInputURLDialog : public CDialogImpl<CTedInputURLDialog> {
public:
	enum {
		IDD = IDD_INPUTURL
	};
...
class CTedCaptureSourceDialog : public CDialogImpl<CTedCaptureSourceDialog> {
public:
	enum {
		IDD = IDD_INPUTCAPSOURCE
	};
...
protected:
	...
	LRESULT OnAddTree(WORD wNotifyCode, WORD wID, HWND hWndCtl, BOOL &bHandled);
	...
	BEGIN_MSG_MAP(CTedApp)
		...
		COMMAND_HANDLER(ID_ADD_TEE, 0, OnAddTree)
...
private:
	...
	bool m_fStopTrackingUintSeesionStarted;
	...
	static const double m_dblInitalSplitterPos;
